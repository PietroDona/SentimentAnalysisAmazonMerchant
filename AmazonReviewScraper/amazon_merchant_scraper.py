...
from AmazonReviewScraper.config import HEADERS, BASEURL_MERCHANT, DELAY
...
class MerchantItemsScraper:
    ...
    def get_page(self, page) -> List[Product]:
        ...
        #
        div_items = soup.find_all("div", {"data-component-type": "s-search-result"})
    ...
    def parse_item(self, item: Tag) -> Product:
        ...
        product_name = (
            item.find("div", class_="s-title-instructions-style").get_text().strip()
        )
        average_review_raw = (
            item.find("i", class_="a-icon").get_text().strip().split(" ")[0]
        )

        self.logger.info(f"Scraped the product with asin {asin}. ")

        return Product(
            asin=asin, title=product_name, average_review=float(average_review_raw)
        )
