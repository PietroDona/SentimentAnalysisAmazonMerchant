...
from AmazonReviewScraper.config import HEADERS, BASEURL_REVIEW, DELAY
...
class ProductReviewScraper:
    ...
    def __init__(
        ...
        if sort not in sort_types:
            raise ValueError(f"Invalid sort type. Expected one of: {sort_types}")
        # if verbose:
    ...
    def get_clean_content(self, item: Tag) -> date:
        ...
        # from here I can extract also the country - not implemented
        content = item.find("span", {"data-hook": "review-body"}).get_text().strip()
        return content

    def get_clean_helpfulvote(self, item: Tag) -> int or None:
        """Clean the number of helpful votes from the HTML"""
        helpfulvote = item.find("span", {"data-hook": "helpful-vote-statement"})
        if not helpfulvote:
            return None
...
if __name__ == "__main__":
    ...
    ars = ProductReviewScraper(
        asin="B07L32B9C2", sort="helpful", max_scrape=2, verbose=True
    )
